...
const index_1 = require("../expressions/index");
...
class FunctionGenerator {
    constructor() {
        this.expressionGenerator = new index_1.ExpressionGenerator();
    }
    /**
    ...
    generate(func) {
        ...
        const params = func.parameters.map(p => `${p.name}: ${index_2.GeneratorUtils.mapTypeToAiken(p.type)}`).join(', ');
        signature += `(${params}) -> ${index_2.GeneratorUtils.mapTypeToAiken(func.returnType)}`;
        lines.push(signature);
        // Convert function body from TypeScript to Aiken syntax
        let aikenBody = index_2.GeneratorUtils.convertFunctionBodyToAiken(func.body);
        ...
        if (func.pipeExpressions && func.pipeExpressions.length > 0) {
            ...
            if (index_2.GeneratorUtils.isConditionalFunction(func.body) && func.pipeExpressions.length > 1) {
    ...
    generateConditionalPipeline(func) {
        if (!func.pipeExpressions || func.pipeExpressions.length === 0) {
            return index_2.GeneratorUtils.convertFunctionBodyToAiken(func.body);
        }
        const lines = [];
        ...
        if (ifMatch) {
            ...
            if (func.pipeExpressions.length >= 1) {
                ...
                if (func.pipeExpressions.length >= 2) {
                    ...
                    const elseBody = index_2.GeneratorUtils.convertFunctionBodyToAiken(elseBranch);
                    lines.push(`  ${elseBody}`);
                }
                lines.push(`}`);
            }
